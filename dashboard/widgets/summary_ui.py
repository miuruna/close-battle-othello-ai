...
def record_table(file_path, enable_select=False):
    data = data_loader.get_record(file_path)
    record = [
    ...
    if enable_select and len(event.selection.rows) > 0:
        selected_index = event.selection.rows[0]
        return selected_index
    return None

def totalization(black, white, progress, share, my_color="black"):
    ...
    if black + white != 0:
        col4.metric("自AIの優勢率", f"{round(share)}%")
    else:
